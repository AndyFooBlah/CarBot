 ...
 * Maps keys but is intentionally public, so we extract it from
 * `.env.local` / `.env.production` and allowlist that exact string. Any
 * other `AIza...` sequence in the bundle is treated as an
 * accidentally-shipped Gemini or Maps key and the build fails.
 *
