...
/**
 * Login page for CarBot.
 * Supports Google OAuth and email/password sign-in and sign-up.
 */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        // Create CarBot profile using the newly authenticated user
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-slate-100 p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-1">Your family's car companion</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4">
          ...
          <button
            ...
            disabled={busy}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-300 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50 transition-colors"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <input
              ...
              disabled={busy}
              className="w-full px-3 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
            />
            <input
              type="password"
